...
bool checkInvalid(float x)
{
	return abs(x - -0.0000000298023) <= 0.0000000000000323877;
}
int find_neighbor(segment *s)
{
	bool sflag = false;
	int count1 = 0, count2 = 0, count3 = 0, count4 = 0;
	if (checkInvalid(s->start->x) || checkInvalid(s->start->y) || checkInvalid(s->start->z))
		sflag = true;
	//	return 0;
	vec3d forward,
			backward;
...
#pragma omp parallel for
		for (int ss = 0; ss < results.size(); ss++)
		{
			segment *s_search = results[ss];
			bool flag = false;
			if (checkInvalid(s_search->start->x) || checkInvalid(s_search->start->y) || checkInvalid(s_search->start->z))
				flag = true;
			//if (checkInvalid(s_search->start->x) || checkInvalid(s_search->start->y) || checkInvalid(s_search->start->z))
			//	continue;
			node start = *(s_search->start);
			node end = *(s_search->end);
					...
					between_start || between_end)
			{
				...
				else if ((between_start || between_end) && !(between_start && between_end))
				{
					if (between_start)
						count3++;
					if (between_end)
			...
			if (start != *(s_search->start) || end != *(s_search->end))
				std::cerr << "COURRPTED AT S_SEARCH #" << s_search->global_index << std::endl;
			if (checkInvalid(s_search->start->x) || checkInvalid(s_search->start->y) || checkInvalid(s_search->start->z))
				if (!flag)
					std::cerr << "INVALID AT S_SEARCH #" << s_search->global_index << std::endl
										<< start << s_search->start;
		}
	...
	//std::cerr << count1 << ", " << count2 << ", " << count3 << ", " << count4 << std::endl;
	//for (int a = 0; a < search_sections * 2; a++)
	//	cout << sections_active[a] << ", ";
	//cout << endl;
	if (sstart != *(s->start) || send != *(s->end))
		std::cerr << "COURRPTED AT S #" << s->global_index << std::endl;
	if (checkInvalid(s->start->x) || checkInvalid(s->start->y) || checkInvalid(s->start->z))
		//if (!sflag)
		std::cerr << "INVALID AT S #" << s->global_index << std::endl;
	return counter;
}
...
#pragma omp parallel for
	for (int search_index = 0; search_index < search_sections * 2; search_index++)
	{
		...
		double pt[3] = {s->middle->x, s->middle->y, s->middle->z};
		auto results = pointLocatorSL.KdTreeFindPointsWithinRadius(SEARCH_LENGTH, pt);

		//get_debug_segment(s);
		//results.push_back(manual_segment);
		//for each search segment in range
		for (int ss = 0; ss < results->GetNumberOfIds(); ss++)
		{
			segment *s_search = all_segments[results->GetId(ss)];
			//for (int t = 0; t < 3; t++)
			for (int t = 1; t == 1; t++)
			{
				float search_x, search_y, search_z;
...
void read_segments_from_sl()
{
	...
	{
		segment *s = segment_starters[i];
		while (s != nullptr &&(int) s != 0xdddddddddddddddd)
		{
			segment *next = s->next;
			delete s;
			s = next;
	...
	for (int i = 0; i < streamlines.size(); i++)
	{
		...
		for (int n = 0; n != streamline->size() - 1; n++)
		{
			...
			//if (n == 0)
			//	std::cout << s->start->x << ", " << (abs(s->start->x - -0.0000000298023) <= 0.0000000000000323877) << std::endl;
...
void update_graph(std::vector<std::vector<node>> &streamlines_)
{
	...
	for (int i = 0; i < segment_starters.size(); i++)
	{
		...
		//TODO: see why first node's xyz is null
		//s = s->next;
		...
		if (((int)counter) % 50 == 0)
		{
			...
			int progress = (counter / segment_starters.size()) * 100;
			cerr << progress << ", ";
		}
	}
	cerr << std::endl;
...
{
	std::vector<segment *> segments = get_segments_in_area(matrix_region_x1, matrix_region_y1, matrix_region_z1, matrix_region_x2, matrix_region_y2, matrix_region_z2);
	size = segments.size();
	std::vector<matrix_entry> entries;
	for (int i = 0; i < segments.size(); i++)
	{
		for (int j = 0; j < segments.size(); j++)
		{
			...
			if (m_edges->get(A->global_index + 1, B->global_index + 1).is_edge)
			{
				//std::cerr << A->global_index << ", " << B->global_index << std::endl;
				matrix_entry entry = {A, B, i, j};
