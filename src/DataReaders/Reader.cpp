...
#include "../SampleDatasets/tornado.h"
#include "../SampleDatasets/bernard.h"
...
void readStreamlines(std::vector<std::vector<node>> &streamlines, std::string name, int skipNodes, int skipLines)
{
	...
	int lineCounter = 0;
	std::string line;
	//std::vector<std::vector<node>> streamline;
	streamlines.clear();
	while (getline(file, line))
	{
		lineCounter++;
		if (skipLines > 1 && lineCounter % skipLines == 0)
			continue;
		float x, y, z;
		...
		{
			nodeCounter++;
			if (skipNodes > 1 && nodeCounter % skipNodes != 0)
				continue;
			...
			if (ln != nullptr)
			{
				...
				n->vy = n->y - ln->y;
				n->vz = n->z - ln->z;
				//float mag = sqrt(n->vx * n->vx + n->vy * n->vy + n->vz * n->vz);
				//cout << mag << endl;
				//std::cout << n->vx << ", " << n->vy << ", " << n->vz << std::endl;
...
void readStreamlines(std::vector<std::vector<node>> &streamlines, vector<vector<Vector3f>> streamlinesV3)
{
	streamlines.clear();
	for (int i = 0; i < streamlinesV3.size(); i++)
	{

		std::vector<node> streamline;
		vector<Vector3f> streamlineV3 = streamlinesV3[i];
		node ln;
		for (int j = 0; j < streamlineV3.size(); j++)
		{
			node n;
			Vector3f v;
			n.x = v[0];
...
profile
		profile_tornado = {PROFILE_TORNADO, 1 / 10., 1 / 5.5},
		profile_bernard = {PROFILE_BERNARD, 1 / 100., 1 / 4.},
